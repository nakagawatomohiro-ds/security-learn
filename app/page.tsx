...
function StageCard({ stage }: { stage: Stage }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ background: "#fff", border: `1.5px solid ${hovered ? color : "#e2e8f0"}`, borderRadius: 12, padding: 24, opacity: stage.status === "locked" ? 0.6 : 1, transform: hovered && stage.status !== "locked" ? "translateY(-2px)" : "none", boxShadow: hovered ? "0 8px 24px rgba(0,0,0,0.08)" : "0 1px 4px rgba(0,0,0,0.04)", transition: "all 0.2s ease", cursor: stage.status === "locked" ? "not-allowed" : "pointer" }} onClick={() => { if (stage.id === 1 && stage.status !== "locked") window.location.href = "/quiz"; }}>
